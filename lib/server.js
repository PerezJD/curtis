...
import { createComponent, isServer, ssrElement, spread, useAssets, ssr, ssrHydrationKey, ssrAttribute, escape, mergeProps as mergeProps$1, Dynamic as Dynamic$1, Portal as Portal$1, ssrStyle, renderToStringAsync } from 'solid-js/web';
import { CacheProvider } from 'emotion-solid';
import { createContext, useContext, mergeProps, splitProps, createMemo, sharedConfig, $DEVCOMP, untrack, createUniqueId as createUniqueId$1, createSignal, $PROXY, $TRACK, getListener, batch, createEffect, createRenderEffect, onCleanup, createComputed, For, onMount, Show, on, children, Switch, Match, mapArray, createComponent as createComponent$1 } from 'solid-js';
...
function resolveBreakpointValues(data) {
  ...
  const base = data.base || computeBreakpointsBase(values, data.breakpoints || {});
...
function deepmerge(target, source, options = {
  ...
  if (isPlainObject(target) && isPlainObject(source)) {
    Object.keys(source).forEach(key => {
      ...
      let sourceValue;
      let targetValue;
      if (isPlainObject(sourceValue = source[key]) && key in target && isPlainObject(targetValue = target[key])) {
...
function merge(target, ...sources) {
  for (const source of sources) deepmerge(target, cloneObject(source), {
    clone: false
  });
  return target;
}
...
function makeVariant(base, fontWeight, size, lineHeight, letterSpacing, casing) {
  return {
...
function createBreakpoints(options) {
  const result = {
    ...
    between: (value, css) => {
      const key = renderMediaQuery("between", [result.resolve(value[0]), result.resolve(value[1])]);
      return css ? {
        [key]: css
      } : key;
    },
...
var getDefaultTheme$1 = makeGetDefaultTheme(createTheme$1);
...
var getDefaultTheme = makeGetDefaultTheme(createTheme);
...
function createStaticComponent(component, props) {
  switch (typeof component) {
    ...
    case "string":
      ...
      const el = sharedConfig.context ? web.getNextElement() : createElement(component, isSvg);
...
function renderSelector(propKey, propValue, selectors = [], options = {}) {
  ...
  return render$1(propValue, (selectors.length ? selectors : [""]).flatMap(selector => subselectors.map(v => v.replace(/&/g, selector).trim())), {
    ...options
  });
}
...
function render$1(css, selectors = [], options = {}) {
  const props = [];
  const rules = [];
  for (let propKey in css) {
    ...
    if (isPrivateVar(propKey)) {
      ...
      rules.push(...render$1(propValue, selectors, {
        ...options,
        sublevel: true
      }).map(v => `${propKey} {\n${v}\n}`));
    } else if (isVar(propKey)) {
      if (propValue !== undefined && propValue !== null) props.push(`${propKey}: ${propValue};`);
    } else if (isKeyframes(propKey)) {
      const keyframes = [];
      for (const k in propValue) {
        keyframes.push(...render$1(propValue[k], [/^\d+$/.test(k) ? `${k}%` : k], {
          ...options,
          sublevel: true
        }));
      }
      ...
      for (const extraPropKey in extraProps) {
        ...
        if (value !== undefined && value !== null) props.push(`${snakeCaseWithCache(extraPropKey, options.propertyNameCache)}: ${value};`);
      }
    } else {
      ...
      if (value !== undefined && value !== null) props.push(`${propKey}: ${value};`);
    }
  }
...
function createStyleObject(options) {
  ...
  const rules = propsValues.map(v => typeof v === "string" ? `.${className} {\n${v}\n}` : render$1(v, [`.${className}`], {
    extraProperties: options.extraProperties,
    propertyNameCache
  }).join("\n")).join("\n");
  return options.cache?.create(options.name, rules, options.componentId) || create(options.name, rules, options.componentId);
}
...
function findStyleElement(input, classPrefix = "css-") {
  ...
  const className = [...input.classList].find(name => name.startsWith(classPrefix));
  if (className) return document.getElementById(className.slice(classPrefix.length));
}
...
function usePreviousProps(props, propNames) {
  ...
  for (const propName of propNames) createEffect(prev => {
...
function setSSRElementAttr(element, name, value) {
  ...
  if (index.attr) {
    element.t = html.slice(0, index.attr.start) + resolveValue(html.slice(index.attr.start, index.attr.end)) + html.slice(index.attr.end);
  } else {
    element.t = html.slice(0, index.tag.end) + ` ${name}="${resolveValue()}"` + html.slice(index.tag.end);
  }
}

function isSSRElement(input) {
  return !!input && typeof input === "object" && "t" in input && typeof input.t === "string" && /^<[a-z]/i.test(input.t.trimStart());
}
...
function addElementClass(input, value) {
  if (isBrowserElement(input)) {
    ...
    setSSRElementAttr(input, "class", prev => prev ? prev.split(" ").includes(value) ? value : `${prev} ${value}` : value);
  }
}
function isSuidElement(input, component) {
  return typeof component.__suid === "string" && hasElementClass(input, component.toString().slice(1));
}

const _tmpl$$q = ["<style", ">", "</style>"];
...
function createStyle(value) {
  ...
  createRenderEffect(prevResult => {
    ...
    if (propsValue) {
      ...
      if (isServer) {
        if (process.env.NODE_ENV !== "production") {
          if (context.injectFirst) console.warn(`SUID: 'injectFirst' is not supported in SSR mode.`);
        }
        useAssets(() => ssr(_tmpl$$q, ssrHydrationKey() + ssrAttribute("id", escape(styleObject.id, true), false) + ssrAttribute("nonce", escape(context.cache?.nonce, true), false), styleObject.rules));
      } else {
  ...
  onCleanup(() => {
    if (styleElement) unregisterStyleElementUsage(styleElement, context.cleanupStyles ?? isGlobalStyleObject);
  });
  return name;
}
 ...
 */
const unitLess = new Set(["animationIterationCount", "borderImageOutset", "borderImageSlice", "borderImageWidth", "boxFlex", "boxFlexGroup", "boxOrdinalGroup", "columnCount", "columns", "flex", "flexGrow", "flexPositive", "flexShrink", "flexNegative", "flexOrder", "gridRow", "gridRowEnd", "gridRowSpan", "gridRowStart", "gridColumn", "gridColumnEnd", "gridColumnSpan", "gridColumnStart", "msGridRow", "msGridRowSpan", "msGridColumn", "msGridColumnSpan", "fontWeight", "lineHeight", "opacity", "order", "orphans", "tabSize", "widows", "zIndex", "zoom", "WebkitLineClamp", "fillOpacity", "floodOpacity", "stopOpacity", "strokeDasharray", "strokeDashoffset", "strokeMiterlimit", "strokeOpacity", "strokeWidth"]);
function resolveStyledPropsValue(name, value) {
  if (typeof value === "number") {
    return {
      [name]: unitLess.has(name) ? value.toString() : `${value}px`
    };
  }
}
...
function mProp(name, suffix, onValue) {
  ...
  return onValue ? (value, theme) => names.reduce((result, name) => {
...
function resolveSxProps(object, theme) {
  return resolve(object, (name, value) => {
    ...
    if (value === null || value === undefined) {
      ...
      return Object.assign(sortedCss, handleBreakpoints({
        theme
      }, value, v => resolveSxPropsValue(name, v, theme) ?? {
        [name]: v
      }));
    } else {
...
function resolveStyles(useTheme, className, styles, inProps) {
  return createMemo(() => {
    ...
    return styles.reduce((result, style) => {
      ...
      if (styledProps) result.push(resolveStyledProps(styledProps, {
        name: className,
        __resolved: true
      }));
      return result;
      ...
      function StyledComponent(inProps) {
        ...
        const $useTheme = () => {
          ...
          if (inTheme) {
            ...
            return theme = config && config.onUseTheme ? config.onUseTheme() : useTheme$2();
          }
        };
        ...
        const $componentType = componentType ? () => componentType : createMemo(() => getSuidComponentType($component()));
        const sx = options.skipSx ? () => inStyles() : () => {
          const theme = $useTheme();
          return [...inStyles(), ...inSx().map(sx => sx.__resolved ? sx : resolveSxProps(sx, theme))];
        };
        ...
        return createDynamicComponent($component, mergeProps(otherProps, {
          ...
          get ownerState() {
            return $componentType() === "system" ? inProps.ownerState : undefined;
          },
          get class() {
            return clsx([...new Set([inProps.class, className, styleClassName()])]);
          }
        }));
      }
...
function isComponentObject(input, component) {
  return !!input && input.$INSPECT === $INSPECT && (!component || input.Component === component);
}
function resolveChildren(children) {
  ...
  if (Array.isArray(children)) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      // eslint-disable-next-line prefer-spread
      results.push.apply(results, result) : results.push(result);
    }
    return results;
  }
  return children;
...
function createComponentFactory() {
  return function (options) {
    function useClasses(ownerState) {
      ...
      const compose = () => {
        if (!options.slotClasses) throw new Error(`'slotClasses' option is not defined`);
        if (!options.utilityClass) throw new Error(`'utilityClass' option is not defined`);
        return composeClasses(options.slotClasses(ownerState), options.utilityClass, ownerState["classes"] ?? "");
      };
      const classes = createMutable({});
      if (haveSlotClasses) createComputed(() => {
...
const Box$1 = defineComponent(function Box(inProps) {
  ...
  return createComponent(BoxRoot, mergeProps$1(inProps, {
    get as() {
...
const Box = $$1w.component(function Box({
  otherProps,
  classes
}) {
  const theme = useTheme$1();
  return createComponent(Box$1, mergeProps$1({
    theme: theme
  }, otherProps, {
    get ["class"]() {
 ...
 */
const Ripple = $$1v.component(function Ripple({
  props,
  otherProps
}) {
  ...
  return createComponent(Box, {
    ...
    get children() {
      return ssr(_tmpl$$p, ssrHydrationKey() + ssrAttribute("class", escape(childClassName(), true), false));
    }
  });
});
...
const TouchRipple = $$1u.defineComponent(function TouchRipple(inProps) {
  ...
  return createComponent(TouchRippleRoot, mergeProps$1({
    get ["class"]() {
    ...
    get children() {
      return createComponent(For, {
        ...
        children: data => createComponent(TouchRippleRipple, {
          get ["in"]() {
...
const ButtonBase = $$1t.defineComponent(function ButtonBase(inProps) {
  ...
  const props = mergeProps({
    ...
    LinkComponent: "a",
    tabIndex: 0
  }, theme.components?.[$$1t.name]?.defaultProps, inProps);
  ...
  const [, otherProps] = splitProps(props, ["LinkComponent", "TouchRippleProps", "action", "centerRipple", "children", "classes", "disableRipple", "disableRipple", "disableTouchRipple", "disabled", "focusRipple", "focusVisibleClassName", "onFocusVisible", "tabIndex", "touchRippleRef"]);
  const ownerState = {
    get classes() {
  ...
  const isNonNativeButton = () => {
    return props.component && props.component !== "button" && !(button.ref.tagName === "A" && button.ref.hasAttribute("href"));
  };
  /**
  ...
  const handleKeyUp = event => {
    ...
    // Keyboard accessibility for non interactive elements
    if (typeof props.onClick === "function" && event.target === event.currentTarget && isNonNativeButton() && event.key === " " && !event.defaultPrevented) {
      props.onClick(event);
    }
  };
  ...
  return createComponent(ButtonBaseRoot, mergeProps$1({
    get type() {
      return isButtonComponent() ? props.type === undefined ? "button" : props.type : undefined;
    },
    get disabled() {
      return isButtonComponent() ? props.disabled : undefined;
    },
    get role() {
      return !isButtonComponent() && props.href && !props.to ? "button" : undefined;
    },
    get ["aria-disabled"]() {
    ...
    get children() {
      return [props.children, createComponent(Show, {
        get when() {
          return enableTouchRipple();
        },
        get children() {
          return createComponent(TouchRipple, mergeProps$1({
            get center() {
...
const $$1s = createComponentFactory()({
  name: "MuiIconButton",
  selfPropNames: ["children", "classes", "color", "disableFocusRipple", "disabled", "edge", "size"],
  propDefaults: ({
    set
  }) => set({
    edge: false,
  ...
  slotClasses: ownerState => ({
    root: ["root", ownerState.disabled && "disabled", ownerState.color !== "default" && `color${capitalize(ownerState.color)}`, ownerState.edge && `edge${capitalize(ownerState.edge)}`, `size${capitalize(ownerState.size)}`]
  })
});
...
const IconButton = $$1s.component(function IconButton({
  ...
  return createComponent(IconButtonRoot, mergeProps$1({
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    centerRipple: true,
...
const Paper = $$1r.component(function Paper({
  ...
  return createComponent(PaperRoot, mergeProps$1(otherProps, {
    ownerState: allProps,
    get ["class"]() {
...
const SvgIcon = $$1q.component(function SvgIcon({
  ...
  return createComponent(SvgIconRoot, mergeProps$1({
    get ["aria-hidden"]() {
    ...
    get children() {
      return [props.children, createComponent(Show, {
        ...
        children: titleAccess => ssr(_tmpl$$o, ssrHydrationKey(), escape(titleAccess()))
      })];
    }
  }));
});
...
function createSvgIcon(path, displayName) {
  const Component = props => createComponent(SvgIcon, mergeProps$1({
...
const _tmpl$$n = ["<path", " d=\"M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z\"></path>"];
/**
 * @ignore - internal component.
 *
 * Alias to `Clear`.
 */
var CloseIcon = createSvgIcon(() => ssr(_tmpl$$n, ssrHydrationKey()), "Close");

const _tmpl$$m = ["<path", " d=\"M11 15h2v2h-2zm0-8h2v6h-2zm.99-5C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8z\"></path>"];
/**
 * @ignore - internal component.
 */
var ErrorOutlineIcon = createSvgIcon(() => ssr(_tmpl$$m, ssrHydrationKey()), "ErrorOutline");

const _tmpl$$l = ["<path", " d=\"M11,9H13V7H11M12,20C7.59,20 4,16.41 4,12C4,7.59 7.59,4 12,4C16.41,4 20,7.59 20, 12C20,16.41 16.41,20 12,20M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10, 10 0 0,0 12,2M11,17H13V11H11V17Z\"></path>"];
/**
 * @ignore - internal component.
 */
var InfoOutlinedIcon = createSvgIcon(() => ssr(_tmpl$$l, ssrHydrationKey()), "InfoOutlined");

const _tmpl$$k = ["<path", " d=\"M12 5.99L19.53 19H4.47L12 5.99M12 2L1 21h22L12 2zm1 14h-2v2h2v-2zm0-6h-2v4h2v-4z\"></path>"];
/**
 * @ignore - internal component.
 */
var ReportProblemOutlinedIcon = createSvgIcon(() => ssr(_tmpl$$k, ssrHydrationKey()), "ReportProblemOutlined");

const _tmpl$$j = ["<path", " d=\"M20,12A8,8 0 0,1 12,20A8,8 0 0,1 4,12A8,8 0 0,1 12,4C12.76,4 13.5,4.11 14.2, 4.31L15.77,2.74C14.61,2.26 13.34,2 12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0, 0 22,12M7.91,10.08L6.5,11.5L11,16L21,6L19.59,4.58L11,13.17L7.91,10.08Z\"></path>"];
/**
 * @ignore - internal component.
 */
var SuccessOutlinedIcon = createSvgIcon(() => ssr(_tmpl$$j, ssrHydrationKey()), "SuccessOutlined");
...
const $$1p = createComponentFactory()({
  name: "MuiAlert",
  selfPropNames: ["action", "classes", "closeText", "color", "icon", "iconMapping", "onClose", "role", "severity", "variant"],
  utilityClass: getAlertUtilityClass,
  slotClasses: ownerState => ({
    root: ["root", `${ownerState.variant}${capitalize(ownerState.color || ownerState.severity)}`, `${ownerState.variant}`],
    icon: ["icon"],
    message: ["message"],
    action: ["action"]
  }),
...
const defaultIconMapping = {
  success: () => createComponent(SuccessOutlinedIcon, {
    fontSize: "inherit"
  }),
  warning: () => createComponent(ReportProblemOutlinedIcon, {
    fontSize: "inherit"
  }),
  error: () => createComponent(ErrorOutlineIcon, {
    fontSize: "inherit"
  }),
  info: () => createComponent(InfoOutlinedIcon, {
    fontSize: "inherit"
  })
};
/**
...
$$1p.component(function Alert({
  ...
  return createComponent(AlertRoot, mergeProps$1({
    get role() {
      return props.role;
    },
    elevation: 0,
      ...
      return [createComponent(Show, {
        ...
        get children() {
          return createComponent(AlertIcon, {
            ...
            get children() {
              return icon() || props.iconMapping?.[props.severity]?.() || defaultIconMapping[props.severity]?.();
            }
          });
        }
...
const Typography = $$1o.defineComponent(function Typography(inProps) {
  ...
  const [, other] = splitProps(props, ["align", "class", "component", "gutterBottom", "noWrap", "paragraph", "variant", "variantMapping"]);
  const ownerState = {
    get classes() {
      return props.classes;
    },
  ...
  const Component = () => props.component || (props.paragraph ? "p" : props.variantMapping[props.variant] || defaultVariantMapping[props.variant]) || "span";
  const classes = $$1o.useClasses(ownerState);
  return createComponent(TypographyRoot, mergeProps$1({
    get as() {
      return Component();
    },
    get sx() {
 ...
 */
$$1n.component(function AlertTitle({
  allProps,
  classes,
  otherProps,
  props
}) {
  return createComponent(AlertTitleRoot, mergeProps$1({
    gutterBottom: true
  }, otherProps, {
    ownerState: allProps,
    get ["class"]() {
...
const $$1m = createComponentFactory()({
  ...
  slotClasses: ownerState => ({
    root: ["root", `color${capitalize(ownerState.color)}`, `position${capitalize(ownerState.position)}`]
  })
});
const AppBarRoot = styled$1(Paper, {
  ...
  slot: "Root",
  overridesResolver: (props, styles) => {
    return [styles.root, styles[`position${capitalize(props.ownerState.position)}`], styles[`color${capitalize(props.ownerState.color)}`]];
  }
})(({
  theme,
  ownerState
...
const AppBar = $$1m.component(function AppBar({
  ...
  return createComponent(AppBarRoot, mergeProps$1({
    square: true,
    ownerState: allProps,
    elevation: 4,
    get ["class"]() {
...
const _tmpl$$i = ["<path", " d=\"M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z\"></path>"];
/**
 * @ignore - internal component.
 */
var Person = createSvgIcon(() => ssr(_tmpl$$i, ssrHydrationKey()), "Person");
 ...
 */
$$1l.defineComponent(function Avatar(inProps) {
  ...
  const children = () => {
    if (hasImgNotFailing()) {
      return createComponent(AvatarImg, mergeProps$1({
        get alt() {
          return props.alt;
        },
        get src() {
  ...
  return createComponent(AvatarRoot, mergeProps$1({
    get as() {
      return baseProps.component;
    },
    ownerState: ownerState,
...
function getTransitionProps(props, options) {
  ...
  return {
    duration: style.transitionDuration ?? (typeof timeout === "number" ? timeout : typeof timeout === "string" ? 0 : timeout[options.mode] || 0),
 ...
 */
const Fade = $$1k.component(function Fade({
  props,
  otherProps
}) {
  const theme = useTheme$1();
 ...
 */
const Backdrop = $$1i.component(function Backdrop({
  props,
  otherProps
}) {
  const [fadeProps, backdropProps] = splitProps(otherProps, fadeSelfPropNames);
  return createComponent(Fade, mergeProps$1({
    get ["in"]() {
      return props.open;
    },
    get timeout() {
    ...
    get children() {
      return createComponent(BackdropUnstyled, mergeProps$1(backdropProps, {
        get ["class"]() {
        ...
        get componentsProps() {
          return {
            root: {
              ...
              ...((!otherProps.components?.Root || !isHostComponent(otherProps.components?.Root)) && {
...
function appendOwnerState(elementType, existingProps, ownerState) {
  const mergedOwnerState = mergeProps(() => existingProps().ownerState || {}, ownerState);
...
function useBadgeInvisibleMemo(props) {
  return createMemo(() => {
    if (props.invisible === false && (props.badgeContent === 0 && !props.showZero || props.badgeContent == null && props.variant !== "dot")) {
      return true;
    } else {
...
function useBadge(inProps) {
  ...
  const prevProps = usePreviousProps(props, ["anchorOrigin", "badgeContent", "max", "variant"]);
  const invisible = useBadgeInvisibleMemo(props);
  const badge = mergeProps(() => invisible() ? prevProps : props);
  const displayValue = () => {
    if (badge.variant !== "dot") {
      return badge.badgeContent && Number(badge.badgeContent) > badge.max ? `${badge.max}+` : badge.badgeContent;
    }
  };
...
const $$1h = createComponentFactory()({
  ...
  autoCallUseClasses: false,
  selfPropNames: ["anchorOrigin", "badgeContent", "children", "classes", "components", "components", "componentsProps", "invisible", "max", "showZero", "variant"],
  propDefaults: ({
    set
  }) => set({
    anchorOrigin: {
  ...
  slotClasses: ownerState => ({
    root: ["root"],
    badge: ["badge", ownerState.variant, `anchorOrigin${capitalize(ownerState.anchorOrigin.vertical)}${capitalize(ownerState.anchorOrigin.horizontal)}`, ownerState.invisible && "invisible"]
  })
});
/**
 *
...
const BadgeUnstyled = $$1h.component(function BadgeUnstyled({
  ...
  return createComponent(Dynamic$1, mergeProps$1(rootProps, otherProps, {
    get component() {
      return Root();
    },
    get ["class"]() {
    ...
    get children() {
      return [props.children, createComponent(Dynamic$1, mergeProps$1(badgeProps, {
        get component() {
          return Badge();
        },
        get ["class"]() {
...
const badgeClasses = Object.assign({}, badgeUnstyledClasses, generateUtilityClasses("MuiBadge", ["colorError", "colorInfo", "colorPrimary", "colorSecondary", "colorSuccess", "colorWarning", "overlapRectangular", "overlapCircular",
...
const $$1g = createComponentFactory()({
  ...
  slotClasses: ownerState => {
    return {
      badge: [`anchorOrigin${capitalize(ownerState.anchorOrigin.vertical)}${capitalize(ownerState.anchorOrigin.horizontal)}${capitalize(ownerState.overlap)}`, `overlap${capitalize(ownerState.overlap)}`, ownerState.color !== "default" && `color${capitalize(ownerState.color)}`]
    };
  }
});
 ...
 */
$$1g.component(function Badge({
  allProps,
  otherProps,
  props
}) {
  const [, otherPropsWithotuComponent] = splitProps(otherProps, ["component"]);
  const prevProps = usePreviousProps(allProps, ["anchorOrigin", "color", "overlap"]);
  ...
  return createComponent(BadgeUnstyled, mergeProps$1({
    get variant() {
      return props.variant;
    }
 ...
 */
$$1f.defineComponent(function BottomNavigation(inProps) {
  const props = $$1f.useThemeProps({
    props: inProps
  });
  const [, other] = splitProps(props, ["children", "class", "component", "onChange", "showLabels", "value"]);
  const baseProps = mergeProps({
    component: "div",
    showLabels: false
  }, props);
  ...
  return createComponent(BottomNavigationRoot, mergeProps$1({
    get as() {
      return baseProps.component;
    },
    get ["class"]() {
...
const $$1e = createComponentFactory()({
  ...
  slotClasses: ownerState => ({
    root: ["root", !ownerState.showLabel && !ownerState.selected && "iconOnly", ownerState.selected && "selected"],
    label: ["label", !ownerState.showLabel && !ownerState.selected && "iconOnly", ownerState.selected && "selected"]
  })
});
 ...
 * - inherits [ButtonBase API](https://mui.com/api/button-base/)
 */
$$1e.defineComponent(function BottomNavigationAction(inProps) {
  const ref = createRef(inProps);
  const props = $$1e.useThemeProps({
    props: inProps
  });
  const [, other] = splitProps(props, ["class", "icon", "label", "onClick", "showLabel", "value"]);
  ...
  return createComponent(BottomNavigationActionRoot, mergeProps$1({
    get ["class"]() {
      return clsx(classes.root, props.class);
    },
    focusRipple: true,
    ...
    get children() {
      return [props.icon, createComponent(BottomNavigationActionLabel, {
        get ["class"]() {
...
const _tmpl$$h = ["<path", " d=\"M6 10c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm12 0c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm-6 0c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z\"></path>"];
/**
 * @ignore - internal component.
 */
var MoreHorizIcon = createSvgIcon(() => ssr(_tmpl$$h, ssrHydrationKey()), "MoreHoriz");

const _tmpl$$g = ["<li", ">", "</li>"];
const BreadcrumbCollapsedButton = styled$1(ButtonBase)(({
  theme
}) => ({
  display: "flex",
...
function BreadcrumbCollapsed(props) {
  return ssr(_tmpl$$g, ssrHydrationKey(), escape(createComponent(BreadcrumbCollapsedButton, mergeProps$1({
    focusRipple: true
  }, props, {
    ownerState: props,
    get children() {
...
const $$1d = createComponentFactory()({
  ...
  selfPropNames: ["children", "classes", "expandText", "itemsAfterCollapse", "itemsBeforeCollapse", "maxItems", "separator"],
...
function insertSeparators(items, className, separator, ownerState) {
  return items.reduce((acc, current, index) => {
    if (index < items.length - 1) {
      acc = acc.concat(current, createComponent(BreadcrumbsSeparator, {
        as: "div",
        "aria-hidden": true,
        "class": className,
        ownerState: ownerState,
 ...
 */
$$1d.component(function Breadcrumbs({
  allProps,
  classes,
  otherProps,
  props
  ...
  const renderItemsBeforeAndAfter = allItems => {
    ...
    if (props.itemsBeforeCollapse + props.itemsAfterCollapse >= allItems.length) {
      if (process.env.NODE_ENV !== "production") {
        console.error(["MUI: You have provided an invalid combination of props to the Breadcrumbs.", `itemsAfterCollapse={${props.itemsAfterCollapse}} + itemsBeforeCollapse={${props.itemsBeforeCollapse}} >= maxItems={${props.maxItems}}`].join("\n"));
      }
      return allItems;
    }
    return [...allItems.slice(0, props.itemsBeforeCollapse), createComponent(BreadcrumbCollapsed, {
      get ["aria-label"]() {
  ...
  const allItems = createMemo(() => {
    ...
    return array.filter(item => (item ?? false) !== false).map(item => ssr(_tmpl$$f, ssrHydrationKey() + ssrAttribute("class", escape(classes.li, true), false), escape(item)));
  });
  return createComponent(BreadcrumbsRoot, mergeProps$1({
    ...
    get children() {
      return createComponent(BreadcrumbsOl, {
        ...
        ownerState: ownerState,
        get children() {
          return insertSeparators(expanded() || props.maxItems && allItems().length <= props.maxItems ? allItems() : renderItemsBeforeAndAfter(allItems()), classes.separator, props.separator, ownerState);
        }
      });
    }
  }));
  ...
  return createComponent(ButtonRoot, mergeProps$1({
    ...
    get children() {
      return [createComponent(Show, {
        ...
        children: startIcon => createComponent(ButtonStartIcon, {
          get ["class"]() {
        ...
        children: endIcon => createComponent(ButtonEndIcon, {
          get ["class"]() {
...
const $$1b = createComponentFactory()({
  name: "MuiButtonGroup",
  selfPropNames: ["children", "classes", "color", "disableElevation", "disableFocusRipple", "disableRipple", "disabled", "fullWidth", "orientation", "size", "variant"],
  propDefaults: ({
    set
  }) => set({
    color: "primary",
  ...
  slotClasses: ownerState => ({
    root: ["root", ownerState.variant, ownerState.orientation === "vertical" && "vertical", ownerState.fullWidth && "fullWidth", ownerState.disableElevation && "disableElevation"],
    grouped: ["grouped", `grouped${capitalize(ownerState.orientation)}`, `grouped${capitalize(ownerState.variant)}`, `grouped${capitalize(ownerState.variant)}${capitalize(ownerState.orientation)}`, `grouped${capitalize(ownerState.variant)}${capitalize(ownerState.color)}`, ownerState.disabled && "disabled"]
  })
});
 ...
 */
$$1b.component(function ButtonGroup({
  allProps,
  classes,
  otherProps,
  props
  ...
  return createComponent(ButtonGroupRoot, mergeProps$1({
    get as() {
 ...
 * - inherits [Paper API](https://mui.com/api/paper/)
 */
const Card = $$1a.component(function Card({
  props,
  otherProps,
  classes
}) {
  return createComponent(CardRoot, mergeProps$1({
    get elevation() {
 ...
 * - inherits [ButtonBase API](https://mui.com/api/button-base/)
 */
$$19.component(function CardActionArea({
  allProps,
  classes,
  otherProps,
  props
}) {
  return createComponent(CardActionAreaRoot, mergeProps$1(otherProps, {
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    get focusVisibleClassName() {
    ...
    ownerState: allProps,
    get children() {
      return [otherProps.children, createComponent(CardActionAreaFocusHighlight, {
        get ["class"]() {
 ...
 */
$$18.component(function CardActions({
  allProps,
  classes,
  otherProps,
  props
}) {
  return createComponent(CardActionsRoot, mergeProps$1(otherProps, {
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    ownerState: allProps,
...
const CardContent = $$17.component(function CardContent({
  ...
  return createComponent(CardContentRoot, mergeProps$1(otherProps, {
    ownerState: allProps,
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
...
const $$16 = createComponentFactory()({
  name: "MuiCardHeader",
  selfPropNames: ["action", "avatar", "classes", "disableTypography", "subheader", "subheaderTypographyProps", "title", "titleTypographyProps"],
  propDefaults: ({
    set
  }) => set({
    component: "div",
 ...
 */
$$16.component(function CardHeader({
  allProps,
  classes,
  otherProps,
  props
  ...
  const Title = () => {
    ...
    return createComponent(Show, {
      get when() {
        return title() !== undefined && !isSuidElement(title(), Typography) && !props.disableTypography;
      },
      get fallback() {
        return title();
      },
      get children() {
        return createComponent(Typography, mergeProps$1({
          get variant() {
  ...
  const Subheader = () => {
    ...
    return createComponent(Show, {
      get when() {
        return subheader() !== undefined && !isSuidElement(subheader(), Typography) && !props.disableTypography;
      },
      get fallback() {
        return subheader();
      },
  ...
  return createComponent(CardHeaderRoot, mergeProps$1(otherProps, {
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    ownerState: allProps,
    get children() {
      return [createComponent(Avatar, {}), createComponent(CardHeaderContent, {
        get ["class"]() {
          return classes.content;
        },
        ownerState: allProps,
        get children() {
          return [createComponent(Title, {}), createComponent(Subheader, {})];
        }
...
const $$15 = createComponentFactory()({
  ...
  slotClasses: ownerState => ({
    root: ["root", ownerState.isMediaComponent && "media", ownerState.isImageComponent && "img"]
  })
});
...
const CardMedia = $$15.component(function CardMedia({
  ...
  const style = () => !isMediaComponent() && props.image ? mergeProps({
    get "background-image"() {
      return `url("${props.image}")`;
    }
  }, typeof otherProps.style === "object" && otherProps.style ? otherProps.style : {}) : otherProps.style;
  const ownerState = mergeProps(allProps, {
  ...
  return createComponent($CardMediaRoot, mergeProps$1({
    get role() {
...
function useControlled(props) {
  ...
  if (isControlled) createEffect(() => {
    setValue(() => value());
  });
  if (process.env.NODE_ENV !== "production") {
    createEffect(() => {
      if (isControlled !== (props.controlled() !== undefined)) {
        console.error([`MUI: A component is changing the ${isControlled ? "" : "un"}controlled ${props.state ?? "value"} state of ${props.name} to be ${isControlled ? "un" : ""}controlled.`, "Elements should not switch from uncontrolled to controlled (or vice versa).", `Decide between using a controlled or uncontrolled ${props.name} ` + "element for the lifetime of the component.", "The nature of the state is determined during the first render. It's considered controlled if the value is not `undefined`.", "More info: https://fb.me/react-controlled-components"].join("\n"));
      }
    });
    let first = true;
    createEffect(on(() => props.default(), () => {
      if (first) {
        ...
        console.error([`MUI: A component is changing the default ${props.state ?? "value"} state of an uncontrolled ${props.name} after being initialized. ` + `To suppress this warning opt to use a controlled ${props.name}.`].join("\n"));
      }
    }));
  }
...
const SwitchBase = $$14.component(function SwitchBase({
  ...
  createEffect(() => {
    if (typeof props.defaultChecked === "boolean") element.ref.defaultChecked = props.defaultChecked;
  });
  createEffect(() => {
    const value = checked();
    if (typeof value === "boolean") element.ref.checked = value;
  });
  return createComponent(SwitchBaseRoot, mergeProps$1(otherProps, {
    component: "span",
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    ...
    ownerState: ownerState,
    get children() {
      return [createComponent(SwitchBaseInput, mergeProps$1({
        as: "input",
        get autofocus() {
...
const _tmpl$$e = ["<path", " d=\"M19 3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.11 0 2-.9 2-2V5c0-1.1-.89-2-2-2zm-9 14l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z\"></path>"];
/**
 * @ignore - internal component.
 */
var CheckBoxIcon = createSvgIcon(() => ssr(_tmpl$$e, ssrHydrationKey()), "CheckBox");

const _tmpl$$d = ["<path", " d=\"M19 5v14H5V5h14m0-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2z\"></path>"];
/**
 * @ignore - internal component.
 */
var CheckBoxOutlineBlankIcon = createSvgIcon(() => ssr(_tmpl$$d, ssrHydrationKey()), "CheckBoxOutlineBlank");

const _tmpl$$c = ["<path", " d=\"M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-2 10H7v-2h10v2z\"></path>"];
/**
 * @ignore - internal component.
 */
var IndeterminateCheckBoxIcon = createSvgIcon(() => ssr(_tmpl$$c, ssrHydrationKey()), "IndeterminateCheckBox");
 ...
 * - inherits [ButtonBase API](https://mui.com/api/button-base/)
 */
$$13.component(function Checkbox({
  allProps,
  classes,
  props
}) {
  ...
  return createComponent(SvgIconContext.Provider, {
    ...
    get children() {
      return createComponent(CheckboxRoot, mergeProps$1({
        type: "checkbox"
      }, baseProps, {
        classes: allClasses,
        get inputProps() {
...
const _tmpl$$b = ["<path", " d=\"M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z\"></path>"];
/**
 * @ignore - internal component.
 */
var CancelIcon = createSvgIcon(() => ssr(_tmpl$$b, ssrHydrationKey()), "Cancel");
...
$$12.component(function Chip({
  ...
  const DeleteIcon = () => {
    ...
    onMount(() => {
      const element = deleteIcon();
      if (element instanceof SVGElement || element instanceof HTMLElement) element.onclick = handleDeleteIconClick;
    });
    return createComponent(Show, {
      get when() {
  ...
  return createComponent($ChipRoot, mergeProps$1({
    get as() {
      return component();
    },
    get ["class"]() {
 ...
 */
$$11.component(function CircularProgress({
  allProps,
  classes,
  otherProps,
  props
  ...
  return createComponent(CircularProgressRoot, mergeProps$1({
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    ownerState: allProps,
...
const $$10 = createComponentFactory()({
  ...
  slotClasses: ownerState => ({
    root: ["root", ownerState.maxWidth && `maxWidth${capitalize(String(ownerState.maxWidth))}`, ownerState.fixed && "fixed", ownerState.disableGutters && "disableGutters"]
  })
});
...
const Container = $$10.component(function Container({
  ...
  return createComponent(ContainerRoot, mergeProps$1(otherProps, {
    get as() {
      return otherProps.component;
    },
    ownerState: allProps,
 ...
 */
const GlobalStyles = $$$.component(function GlobalStyles({
  props
}) {
  createStyle(() => ({
    "@global": props.styles || {}
 ...
 */
const CssBaseline = $$_.component(function CssBaseline({
  props
}) {
  const theme = useTheme$1();
...
function handleContainer(containerInfo, props) {
  ...
  if (!props.disableScrollLock) {
    ...
    const scrollContainer = parent?.nodeName === "HTML" && containerWindow.getComputedStyle(parent).overflowY === "scroll" ? parent : container;
      ...
      return createComponent(Show, {
        ...
        get children() {
          return createComponent(Portal, {
            ...
            get children() {
              return createComponent(Dynamic$1, mergeProps$1(otherProps, {
                get component() {
                  return Root();
                },
                role: "presentation"
                ...
                get children() {
                  return [createComponent(Show, {
                    ...
                    get children() {
                      return createComponent(Dynamic$1, mergeProps$1({
                        get component() {
...
const Modal = $$Y.defineComponent(function Modal(inProps) {
  ...
  return createComponent(ModalUnstyled, mergeProps$1({
    get components() {
      return mergeProps({
        Root: ModalRoot
 ...
 */
$$X.defineComponent(function Dialog(inProps) {
  ...
  return createComponent(DialogRoot, mergeProps$1({
    get ["class"]() {
      return clsx(classes.root, props.class);
    },
    get BackdropProps() {
    ...
    get children() {
      return createComponent(baseProps.TransitionComponent, mergeProps$1({
        appear: true,
        get ["in"]() {
          return props.open;
        },
        ...
        get children() {
          return createComponent(DialogContainer, {
            ...
            ownerState: ownerState,
            get children() {
              return createComponent(DialogPaper, mergeProps$1({
                get component() {
 ...
 */
$$W.defineComponent(function DialogActions(inProps) {
  ...
  return createComponent(DialogActionsRoot, mergeProps$1({
    get ["class"]() {
      return clsx(classes.root, props.class);
    },
    ownerState: ownerState
 ...
 */
$$V.defineComponent(function DialogContent(inProps) {
  ...
  return createComponent(DialogContentRoot, mergeProps$1({
    get ["class"]() {
      return clsx(classes.root, props.class);
    },
    ownerState: ownerState
 ...
 */
$$U.defineComponent(function DialogContentText(inProps) {
  const props = $$U.useThemeProps({
    props: inProps
  });
  const [, ownerState] = splitProps(props, ["children"]);
  const classes = $$U.useClasses(ownerState);
  return createComponent(DialogContentTextRoot, mergeProps$1({
    variant: "body1",
    color: "text.secondary",
    ownerState: ownerState
  }, props, {
 ...
 */
$$T.defineComponent(function DialogTitle(inProps) {
  ...
  return createComponent(DialogTitleRoot, mergeProps$1({
    get ["class"]() {
      return clsx(classes.root, props.class);
    },
    ownerState: ownerState,
...
const $$S = createComponentFactory()({
  name: "MuiDivider",
  selfPropNames: ["absolute", "children", "classes", "flexItem", "light", "orientation", "textAlign", "variant"],
  propDefaults: ({
    set,
    inProps
  }) => set({
  ...
  slotClasses: o => ({
    root: ["root", o.absolute && "absolute", o.variant, o.light && "light", o.orientation === "vertical" && "vertical", o.flexItem && "flexItem", !!o.children && "withChildren", !!o.children && o.orientation === "vertical" && "withChildrenVertical", o.textAlign === "right" && o.orientation !== "vertical" && "textAlignRight", o.textAlign === "left" && o.orientation !== "vertical" && "textAlignLeft"],
 ...
 */
$$S.component(function Divider({
  allProps,
  otherProps,
  classes
}) {
  const $DividerRoot = redefine(DividerRoot, "div", "hr");
  return createComponent($DividerRoot, mergeProps$1({
    get role() {
    ...
    get children() {
      return createComponent(Show, {
        ...
        children: children => createComponent(DividerWrapper, {
          get ["class"]() {
...
const $$R = createComponentFactory()({
  name: "MuiSlide",
  selfPropNames: ["appear", "children", "container", "direction", "easing", "in", "timeout"],
  propDefaults: ({
    set
  }) => {
    const theme = useTheme$1();
 ...
 * - inherits [Transition API](http://reactcommunity.org/react-transition-group/transition/#Transition-props)
 */
const Slide = $$R.component(function Slide({
  props,
  otherProps
}) {
  const theme = useTheme$1();
  ...
  createEffect(prev => {
    ...
    if (!props.in) setTranslateValue(props.direction, resolved(), props.container);
...
const $$Q = createComponentFactory()({
  name: "MuiDrawer",
  selfPropNames: ["ModalProps", "PaperProps", "anchor", "children", "classes", "elevation", "onClose", "open", "transitionDuration", "variant"],
  utilityClass: getDrawerUtilityClass,
  slotClasses: ownerState => ({
    root: ["root"],
    docked: [(ownerState.variant === "permanent" || ownerState.variant === "persistent") && "docked"],
    modal: ["modal"],
    paper: ["paper", `paperAnchor${capitalize(ownerState.anchor)}`, ownerState.variant !== "temporary" && `paperAnchorDocked${capitalize(ownerState.anchor)}`]
  }),
  propDefaults: ({
    set
  }) => {
...
$$Q.component(function Drawer({
  ...
  function InternalDrawer() {
    return createComponent(DrawerPaper, mergeProps$1({
      get elevation() {
  ...
  function SlidingDrawer() {
    ...
    return createComponent(Slide, mergeProps$1({
      get ["in"]() {
        return props.open;
      },
      get direction() {
  ...
  return createComponent(Switch, {
    get children() {
      return [createComponent(Match, {
        ...
        get children() {
          return createComponent(DrawerDockedRoot, mergeProps$1(otherProps, {
            get ["class"]() {
        ...
        get children() {
          return createComponent(DrawerRoot, mergeProps$1({
            get BackdropProps() {
              return {
            ...
            get ["class"]() {
              return clsx(classes.root, classes.modal, otherProps.class);
            },
            get open() {
              return props.open;
            },
...
const $$P = createComponentFactory()({
  name: "MuiFab",
  selfPropNames: ["children", "classes", "color", "disabled", "disableFocusRipple", "disableRipple", "href", "size", "variant"],
  utilityClass: getFabUtilityClass,
  slotClasses: ownerState => ({
    root: ["root", ownerState.variant, `size${capitalize(ownerState.size)}`, ownerState.color === "inherit" ? "colorInherit" : ownerState.color]
  })
});
const FabRoot = styled$1(ButtonBase, {
  ...
  ...(ownerState.color !== "inherit" && ownerState.color !== "default" && theme.palette[ownerState.color] != null && {
    color: theme.palette[ownerState.color].contrastText,
    backgroundColor: theme.palette[ownerState.color].main,
    "&:hover": {
      backgroundColor: theme.palette[ownerState.color].dark,
 ...
 * - inherits [ButtonBase API](https://mui.com/api/button-base/)
 */
$$P.defineComponent(function Fab(inProps) {
  const props = $$P.useThemeProps({
    props: inProps
  });
  const [, other] = splitProps(props, ["children", "class", "color", "component", "disabled", "disableFocusRipple", "focusVisibleClassName", "size", "variant"]);
  const baseProps = mergeProps({
    color: "default",
    component: "button",
    disabled: false,
  ...
  return createComponent(FabRoot, mergeProps$1({
    get ["class"]() {
      return clsx(classes.root, props.class);
    },
    get component() {
...
const _tmpl$$a = ["<textarea", " aria-hidden", " readonly", " style=\"", "\"></textarea>"];
...
const TextareaAutosize = $$O.defineComponent(function (props) {
  ...
  return [ssrElement("textarea", mergeProps$1({
    get rows() {
      return baseProps.minRows;
    },
    style: style1
  }, other), undefined, true), ssr(_tmpl$$a, ssrHydrationKey(), ssrAttribute("class", escape(props.class, true), false), ssrAttribute("tabindex", -1, false), ssrStyle(style2))];
}, false);
...
const inputGlobalStyles = () => createComponent(GlobalStyles, {
  styles: {
...
const InputBase = $$N.component(function InputBase({
  ...
  const hasSelectionRange = () => inputRef.ref.nodeName === "TEXTAREA" || !inputRef.ref.type || selectionTypes.has(inputRef.ref.type);
  ...
  onMount(() => {
    ...
    if (isControlled && isElement) inputRef.ref.addEventListener("blur", () => {
      if (attrValueOnBlur()) {
    ...
    if (isControlled || loadDefaultValue) {
      ...
      if (isInputObject) {
        ...
        if (hasSelectionRange()) {
          ...
          if (input.selectionStart !== selectionStart) input.setSelectionRange(selectionStart, selectionStart);
        }
      }
    }
    return false;
  ...
  return [!props.disableInjectingGlobalStyles && inputGlobalStyles(), createComponent(Dynamic, mergeProps$1(rootProps, otherProps, {
    get $component() {
      return Root();
    }
    ...
    get children() {
      return [props.startAdornment, createComponent(FormControlContext.Provider, {
        value: undefined,
        get children() {
          return createComponent(Dynamic, mergeProps$1({
            get $component() {
...
const FilledInput = $$M.component(function FilledInput({
  ...
  const allClasses = mergeProps(() => props.classes || {}, classes);
  return createComponent(InputBase, mergeProps$1(otherProps, {
    get components() {
      return {
...
const FormControl = $$L.component(function FormControl({
  ...
  const focused = () => props.focused !== undefined && !props.disabled ? props.focused : focusedState();
  let registerEffect;
  if (process.env.NODE_ENV !== "production") {
  ...
  return createComponent(FormControlContext.Provider, {
    ...
    get children() {
      return createComponent(FormControlRoot, mergeProps$1(otherProps, {
        ownerState: allProps,
        get ["class"]() {
          return clsx(classes.root, otherProps.class);
        },
 ...
 */
$$K.component(function FormControlLabel({
  allProps,
  otherProps,
  props
}) {
  ...
  return createComponent(FormControlLabelContext.Provider, {
    value: contextProps,
    get children() {
      return createComponent(FormControlLabelRoot, mergeProps$1(otherProps, {
        get ["class"]() {
          return clsx(classes.root, otherProps.class);
        },
        ownerState: ownerState,
        get children() {
          return [props.control, createComponent(Label, {})];
        }
      }));
    }
  });
...
const FormGroup = $$J.component(function FormGroup({
  ...
  return createComponent(FormGroupRoot, mergeProps$1(otherProps, {
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    ownerState: ownerState,
...
const _tmpl$$9 = ["<span", " class=\"notranslate\">&#8203;</span>"];
const $$I = createComponentFactory()({
  ...
  selfPropNames: ["children", "classes", "disabled", "error", "filled", "focused", "margin", "required", "variant"],
  utilityClass: getFormHelperTextUtilityClasses,
  autoCallUseClasses: false,
  slotClasses: ownerState => ({
    root: ["root", !!ownerState.disabled && "disabled", !!ownerState.error && "error", !!ownerState.size && `size${capitalize(ownerState.size)}`, ownerState.contained && "contained", !!ownerState.focused && "focused", !!ownerState.filled && "filled", !!ownerState.required && "required"]
  })
});
...
const FormHelperText = $$I.component(function FormHelperText({
  ...
  const resolved = children(() => props.children);
  return createComponent(FormHelperTextRoot, mergeProps$1(otherProps, {
    get as() {
      return otherProps.component;
    },
    ownerState: ownerState,
    ...
    get children() {
      return resolved() === " " ? // notranslate needed while Google Translate will not fix zero-width space issue
      ssr(_tmpl$$9, ssrHydrationKey()) : resolved();
    }
  }));
});
...
const FormLabel = $$H.component(function FormLabel({
  ...
  return createComponent(FormLabelRoot, mergeProps$1(otherProps, {
    get as() {
      return otherProps.component;
    },
    ownerState: ownerState,
...
const Grid = $$G.component(function Grid({
  ...
  const RootElement = () => createComponent(GridRoot, mergeProps$1({
    ownerState: allProps
  }, otherProps, {
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
 ...
 * It uses [react-transition-group](https://github.com/reactjs/react-transition-group) internally.
 */
const Grow = $$F.component(function Grow({
  props,
  otherProps
}) {
  const autoTimeout = {
  ...
  return createComponent(Transition, mergeProps$1(otherProps, {
    get appear() {
      return props.appear;
    },
    get ["in"]() {
...
const $$E = createComponentFactory()({
  ...
  slotClasses: ownerState => ({
    root: ["root", ownerState.color !== "inherit" && `color${capitalize(ownerState.color)}`, `fontSize${capitalize(ownerState.fontSize)}`]
  })
});
 ...
 */
$$E.component(function Icon({
  allProps,
  classes,
  otherProps,
  props
}) {
  return createComponent(IconRoot, mergeProps$1({
    get ["class"]() {
      return clsx(props.baseClassName,
      // Prevent the translation of the text content.
      // The font relies on the exact text content to render the icon.
      "notranslate", classes.root, otherProps.class);
    },
    ownerState: allProps,
    "aria-hidden": true,
    get children() {
...
const Input = $$D.component(function Input({
  classes,
  otherProps,
  props
}) {
...
const _tmpl$$8 = ["<span", " class=\"notranslate\">&#8203;</span>"];
const $$C = createComponentFactory()({
  name: "MuiInputAdornment",
  selfPropNames: ["classes", "children", "disablePointerEvents", "disableTypography", "position", "variant"],
  utilityClass: getInputAdornmentUtilityClass,
  slotClasses: ownerState => ({
    root: ["root", ownerState.disablePointerEvents && "disablePointerEvents", ownerState.position && `position${capitalize(ownerState.position)}`, ownerState.variant, ownerState.hiddenLabel && "hiddenLabel", ownerState.size && `size${capitalize(ownerState.size)}`]
  })
});
 ...
 */
$$C.defineComponent(function InputAdornment(inProps) {
  const props = $$C.useThemeProps({
    props: inProps
  });
  const [, other] = splitProps(props, ["children", "class", "component", "disablePointerEvents", "disableTypography", "position", "variant"]);
  const baseProps = mergeProps({
    component: "div",
  ...
  function RootChildren(props) {
    ...
    return createComponent(Show, {
      ...
      get fallback() {
        return [props.position === "start" ? /* notranslate needed while Google Translate will not fix zero-width space issue */ssr(_tmpl$$8, ssrHydrationKey()) : null, props.children];
      },
      get children() {
  ...
  return createComponent(FormControlContext.Provider, {
    value: undefined,
    get children() {
      return createComponent(InputAdornmentRoot, mergeProps$1({
        get as() {
          return baseProps.component;
        },
        ownerState: ownerState,
 ...
 */
const InputLabel = $$B.component(function InputLabel({
  allProps,
  props
}) {
  const muiFormControl = useFormControl();
  ...
  const allClasses = mergeProps(classes, () => props.classes || {});
  return createComponent(InputLabelRoot, mergeProps$1(baseProps, {
    get ["data-shrink"]() {
      return shrink();
    },
    ownerState: ownerState,
...
const $$A = createComponentFactory()({
  ...
  slotClasses: ownerState => ({
    ...
    bar1: ["bar", `barColor${capitalize(ownerState.color)}`, (ownerState.variant === "indeterminate" || ownerState.variant === "query") && "bar1Indeterminate", ownerState.variant === "determinate" && "bar1Determinate", ownerState.variant === "buffer" && "bar1Buffer"],
    bar2: ["bar", ownerState.variant !== "buffer" && `barColor${capitalize(ownerState.color)}`, ownerState.variant === "buffer" && `color${capitalize(ownerState.color)}`, (ownerState.variant === "indeterminate" || ownerState.variant === "query") && "bar2Indeterminate", ownerState.variant === "buffer" && "bar2Buffer"]
  })
});
 ...
 */
$$A.component(function LinearProgress({
  allProps,
  classes,
  otherProps,
  props
  ...
  return createComponent($LinearProgressRoot, mergeProps$1({
    role: "progressbar"
  }, otherProps, {
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
...
const $$z = createComponentFactory()({
  name: "MuiLink",
  autoCallUseClasses: false,
  selfPropNames: ["TypographyClasses", "children", "classes", "color", "underline", "variant"],
  propDefaults: ({
    set
  }) => set({
    color: "primary",
  ...
  slotClasses: ownerState => ({
    root: ["root", `underline${capitalize(ownerState.underline)}`, ownerState.component === "button" && "button", ownerState.focusVisible && "focusVisible"]
  })
});
 ...
 * - inherits [Typography API](https://mui.com/api/typography/)
 */
const Link = $$z.component(function Link({
  allProps,
  otherProps,
  props
}) {
...
const List = $$y.component(function List({
  ...
  return createComponent(ListContext.Provider, {
    ...
    get children() {
      return createComponent(ListRoot, mergeProps$1(otherProps, {
        get ["class"]() {
          return clsx(classes.root, otherProps.class);
        },
        ownerState: allProps,
 ...
 * - inherits [ButtonBase API](https://mui.com/api/button-base/)
 */
$$x.defineComponent(function ListItemButton(inProps) {
  const theme = useTheme$1();
  const props = mergeProps({
    alignItems: "center",
    component: "div"
  }, theme.components?.[$$x.name]?.defaultProps, inProps);
  const [, other] = splitProps(props, ["alignItems", "autoFocus", "component", "children", "dense", "disableGutters", "divider", "focusVisibleClassName", "selected"]);
  ...
  return createComponent(ListContext.Provider, {
    value: childContext,
    get children() {
      return createComponent(ListItemButtonRoot, mergeProps$1({
        get component() {
          return props.component;
        },
        get focusVisibleClassName() {
          return clsx(props.classes?.focusVisible, props.focusVisibleClassName);
        },
        ownerState: ownerState
      }, other, {
        classes: classes,
...
const ListItemSecondaryAction = $$w.component(function ListItemSecondaryAction({
  ...
  const classes = $$w.useClasses(ownerState);
  return createComponent(ListItemSecondaryActionRoot, mergeProps$1(otherProps, {
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    ownerState: ownerState,
 ...
 */
$$v.defineComponent(function ListItem(inProps) {
  const theme = useTheme$1();
  const baseProps = mergeProps({
    alignItems: "center",
    components: {},
    componentsProps: {}
  }, theme.components?.[$$v.name]?.defaultProps, inProps);
  ...
  const classes = $$v.useClasses(ownerState);
  const [, other] = splitProps(inProps, ["alignItems", "autoFocus", "children", "class", "component", "components", "componentsProps", "dense", "disableGutters", "disablePadding", "divider", "secondaryAction"]);
  return createComponent(ListContext.Provider, {
    value: childContext,
    get children() {
      return createComponent(ListItemRoot, mergeProps$1({
        get as() {
        ...
        get children() {
          return [inProps.children, createComponent(Show, {
            get when() {
              return inProps.secondaryAction;
            },
            children: secondaryAction => createComponent(ListItemSecondaryAction, {
              get children() {
 ...
 */
$$u.component(function ListItemAvatar({
  allProps,
  classes,
  otherProps
}) {
  ...
  return createComponent(ListItemAvatarRoot, mergeProps$1(otherProps, {
    ownerState: ownerState,
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
 ...
 */
$$t.component(function ListItemIcon({
  allProps,
  classes,
  otherProps,
  props
  ...
  return createComponent(ListItemIconRoot, mergeProps$1(otherProps, {
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    ownerState: ownerState,
 ...
 */
$$s.defineComponent(function ListItemText(inProps) {
  const theme = useTheme$1();
  const context = useListContext();
  ...
  const [, otherProps] = splitProps(props, ["children", "classes", "disableTypography", "inset", "primary", "primaryTypographyProps", "secondary", "secondaryTypographyProps"]);
  ...
  const Primary = () => {
    ...
    return createComponent(Show, {
      get when() {
        return isDefined($primary()) && !isSuidElement($primary(), Typography) && !props.disableTypography;
      },
      get fallback() {
        return $primary();
      },
  ...
  const Secondary = () => {
    ...
    return createComponent(Show, {
      get when() {
        return isDefined(secondary()) && !isSuidElement(secondary(), Typography) && !props.disableTypography;
      },
      get fallback() {
        return secondary();
      },
  ...
  return createComponent(ListItemTextRoot, mergeProps$1(otherProps, {
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    ownerState: ownerState,
...
const $$r = createComponentFactory()({
  ...
  selfPropNames: ["children", "classes", "color", "disableGutters", "disableSticky", "inset"],
  propDefaults: ({
    set
  }) => set({
    color: "default",
  ...
  slotClasses: ownerState => ({
    root: ["root", ownerState.color !== "default" && `color${capitalize(ownerState.color)}`, !ownerState.disableGutters && "gutters", ownerState.inset && "inset", !ownerState.disableSticky && "sticky"]
  })
});
 ...
 */
$$r.component(function ListSubheader({
  allProps,
  classes,
  otherProps,
  props
}) {
  return createComponent(ListSubheaderRoot, mergeProps$1(otherProps, {
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    ownerState: allProps,
...
const $$q = createComponentFactory()({
  name: "MuiMenuList",
  selfPropNames: ["autoFocus", "autoFocusItem", "children", "disabledItemsFocusable", "disableListWrap", "variant"]
});
...
const MenuList = $$q.defineComponent(function MenuList(props) {
  ...
  const [, other] = splitProps(props, ["autoFocus", "autoFocusItem", "children", "class", "disabledItemsFocusable", "disableListWrap", "onKeyDown", "variant"]);
  const baseProps = mergeProps({
    autoFocus: false,
    autoFocusItem: false,
  ...
  const handleKeyDown = event => {
    ...
    if (key === "ArrowDown") {
      ...
      if (criteria.previousKeyMatched && (keepFocusOnCurrent || moveFocus(list, currentFocus, false, baseProps.disabledItemsFocusable, nextItem, criteria))) {
        event.preventDefault();
      } else {
  ...
  const items = mapArray(children, (child, index) => {
    if (isComponentObject(child)) {
      return createComponent(child.Component, mergeProps(child.props, {
        get autoFocus() {
          if (index() === activeItemIndex() && baseProps.autoFocusItem) return true;
        },
        get tabIndex() {
          if (child.props.tabIndex === undefined && index() === activeItemIndex() && baseProps.variant) return 0;
        }
      }));
    } else {
      return child;
...
const $$p = createComponentFactory()({
  name: "MuiPopover",
  selfPropNames: ["action", "anchorEl", "anchorOrigin", "anchorPosition", "anchorReference", "children", "classes", "container", "elevation", "marginThreshold", "onClose", "open", "PaperProps", "transformOrigin", "TransitionComponent", "transitionDuration", "TransitionProps"],
...
const Popover = $$p.defineComponent(function Popover(inProps) {
  ...
  const [, other] = splitProps(props, ["action", "anchorEl", "anchorOrigin", "anchorPosition", "anchorReference", "children", "class", "container", "elevation", "marginThreshold", "open", "PaperProps", "transformOrigin", "TransitionComponent", "transitionDuration", "TransitionProps"]);
  const baseProps = mergeProps({
    anchorOrigin: {
      vertical: "top",
      horizontal: "left"
  ...
  const getAnchorOffset = () => {
    ...
    return {
      top: anchorRect.top + getOffsetTop(anchorRect, baseProps.anchorOrigin.vertical),
      left: anchorRect.left + getOffsetLeft(anchorRect, baseProps.anchorOrigin.horizontal)
    };
  };
  ...
  const transitionDuration = () => {
    ...
    if (baseProps.transitionDuration === "auto" && !baseProps.TransitionComponent.muiSupportAuto) {
  ...
  const container = () => props.container || (props.anchorEl ? ownerDocument(resolveAnchorEl$1(props.anchorEl)).body : undefined);
  return createComponent(PopoverRoot, mergeProps$1({
    BackdropProps: {
      invisible: true
    },
    get ["class"]() {
    ...
    get children() {
      return createComponent(baseProps.TransitionComponent, mergeProps$1({
        ...
        get children() {
          return createComponent(PopoverPaper, mergeProps$1({
            get elevation() {
              return baseProps.elevation;
            }
          }, () => baseProps.PaperProps, {
...
const $$o = createComponentFactory()({
  name: "MuiMenu",
  selfPropNames: ["anchorEl", "autoFocus", "children", "classes", "disableAutoFocusItem", "MenuListProps", "onClose", "open", "PopoverClasses", "transitionDuration", "TransitionProps", "variant"],
...
const Menu = $$o.defineComponent(function Menu(inProps) {
  ...
  const [, other] = splitProps(props, ["autoFocus", "children", "disableAutoFocusItem", "MenuListProps", "onClose", "open", "PaperProps", "PopoverClasses", "transitionDuration", "TransitionProps", "variant"]);
  const baseProps = mergeProps({
    autoFocus: true,
  ...
  return createComponent(MenuRoot, mergeProps$1({
    ...
    get children() {
      return createComponent(MenuMenuList, mergeProps$1({
        onKeyDown: handleListKeyDown,
        get autoFocus() {
          return baseProps.autoFocus && (activeItemIndex() === -1 || baseProps.disableAutoFocusItem);
        },
        get autoFocusItem() {
...
const $$n = createComponentFactory()({
  name: "MuiMenuItem",
  selfPropNames: ["autoFocus", "classes", "dense", "disabled", "disableGutters", "divider", "selected"],
  utilityClass: getMenuItemUtilityClass,
  slotClasses: ownerState => ({
    root: ["root", ownerState.dense && "dense", ownerState.disabled && "disabled", !ownerState.disableGutters && "gutters", ownerState.divider && "divider", ownerState.selected && "selected"]
  })
});
...
const MenuItem = $$n.defineComponent(function MenuItem(inProps) {
  ...
  const [, other] = splitProps(props, ["autoFocus", "component", "dense", "divider", "disableGutters", "focusVisibleClassName", "role", "tabIndex"]);
  const baseProps = mergeProps({
    autoFocus: false,
    component: "li",
    dense: false,
  ...
  return createComponent(ListContext.Provider, {
    value: childContext,
    get children() {
      return createComponent(MenuItemRoot, mergeProps$1({
        get role() {
        ...
        get focusVisibleClassName() {
          return clsx(ownerState.classes?.focusVisible, props.focusVisibleClassName);
        }
      }, other, {
        ownerState: ownerState,
        classes: classes
...
const _tmpl$$7 = ["<path", " d=\"M7 10l5 5 5-5z\"></path>"];
/**
 * @ignore - internal component.
 */
var ArrowDropDownIcon = createSvgIcon(() => ssr(_tmpl$$7, ssrHydrationKey()), "ArrowDropDown");
...
const useUtilityClasses = ownerState => {
  const slots = {
    select: ["select", ownerState.variant, ownerState.disabled && "disabled", ownerState.multiple && "multiple"],
    icon: ["icon", ownerState.variant && `icon${capitalize(ownerState.variant)}`, ownerState.open && "iconOpen", ownerState.disabled && "disabled"]
  };
...
const NativeSelectInput = function NativeSelectInput(props) {
  const [, other] = splitProps(props, ["class", "disabled", "IconComponent", "inputRef", "variant"]);
  const baseProps = mergeProps({
    variant: "standard"
  }, props);
  const ownerState = mergeProps(props, {
  ...
  return [createComponent(NativeSelectSelect, mergeProps$1({
    ownerState: ownerState,
    get ["class"]() {
...
const $$m = createComponentFactory()({
  name: "MuiNativeSelect",
  selfPropNames: ["children", "classes", "IconComponent", "input", "inputProps", "onChange", "value", "variant"],
 ...
 */
$$m.defineComponent(function NativeSelect(inProps) {
  const props = $$m.useThemeProps({
    props: inProps
  });
  const [, other] = splitProps(props, ["class", "children", "classes", "IconComponent", "input", "inputProps", "variant"]);
  const baseProps = mergeProps({
    classes: {},
  ...
  return createComponent(Input, mergeProps$1({
...
const _tmpl$$6 = ["<span", ">", "</span>"],
  _tmpl$2 = ["<span", " class=\"notranslate\">&#8203;</span>"];
const $$l = createComponentFactory()({
...
const NotchedOutline = $$l.component(function NotchedOutline({
  ...
  return createComponent(NotchedOutlineRoot$1, mergeProps$1(otherProps, {
    "aria-hidden": true,
    get ["class"]() {
      return otherProps.class;
    },
      ...
      return createComponent(NotchedOutlineLegend, {
        ...
        get children() {
          return createComponent(Show, {
            ...
            get fallback() {
              return (// notranslate needed while Google Translate will not fix zero-width space issue
                ssr(_tmpl$2, ssrHydrationKey())
              );
            },
            get children() {
              return ssr(_tmpl$$6, ssrHydrationKey(), escape(props.label));
            }
          });
        }
      });
...
const OutlinedInput = $$k.component(function OutlinedInput({
  ...
  const label = createMemo(() => {
    ...
    return label != null && label !== "" && fcs.required ? [label, "\xA0", "*"] : label;
  });
  const allClasses = mergeProps(classes, () => props.classes || {}, {
    notchedOutline: null
  });
  return createComponent(InputBase, mergeProps$1({
    renderSuffix: state => createComponent(NotchedOutlineRoot, {
      get ["class"]() {
      ...
      get notched() {
        return typeof props.notched !== "undefined" ? props.notched : Boolean(state.startAdornment || state.filled || state.focused);
      }
    })
  }, otherProps, {
    get components() {
...
const $$j = createComponentFactory()({
  ...
  selfPropNames: ["anchorEl", "children", "container", "direction", "disablePortal", "keepMounted", "modifiers", "open", "placement", "popperOptions", "popperRef", "ref", "transition"],
  propDefaults: ({
    set
  }) => set({
    direction: "ltr",
...
const PopperTooltip = function PopperTooltip(inProps) {
  const [props, otherProps] = splitProps(inProps, ["anchorEl", "children", "direction", "disablePortal", "modifiers", "open",
  //"ownerState",
  "TransitionProps", "placement", "popperOptions", "popperRef"]);
  ...
  return ssrElement("div", mergeProps$1({
    role: "tooltip"
  }, otherProps), () => typeof props.children === "function" ? escape(props.children(childProps())) : escape(props.children), true);
};
/**
...
const PopperUnstyled = $$j.component(function PopperUnstyled({
  ...
  // If the anchorEl prop is provided, use its parent body element as the container
  // If neither are provided let the Modal take care of choosing the container
  const container = createMemo(() => props.container || (props.anchorEl ? ownerDocument(resolveAnchorEl(props.anchorEl)).body : undefined));
  return createComponent(TransitionContext.Provider, {
    value: {
      get in() {
        return props.transition && props.open;
      ...
      return createComponent(Show, {
        ...
        get children() {
          return createComponent(Portal, {
            ...
            get children() {
              return createComponent(PopperTooltip, mergeProps$1({
                get anchorEl() {
                ...
                get open() {
                  return props.transition ? !exited() : props.open;
                },
                get placement() {
                ...
                get style() {
                  return {
                    ...
                    top: 0,
                    left: 0,
                    display: !props.open && props.keepMounted && (!props.transition || exited()) ? "none" : null,
                    ...(otherProps.style && typeof otherProps.style !== "string" ? otherProps.style : {})
                  };
                },
                get children() {
                  return props.children;
 ...
 */
$$i.component(function Popper({
  otherProps
}) {
  const theme = useTheme$1();
  return createComponent(PopperUnstyled, mergeProps$1({
    get direction() {
...
const _tmpl$$5 = ["<path", " d=\"M8.465 8.465C9.37 7.56 10.62 7 12 7C14.76 7 17 9.24 17 12C17 13.38 16.44 14.63 15.535 15.535C14.63 16.44 13.38 17 12 17C9.24 17 7 14.76 7 12C7 10.62 7.56 9.37 8.465 8.465Z\"></path>"];
/**
 * @ignore - internal component.
 */
var RadioButtonCheckedIcon = createSvgIcon(() => ssr(_tmpl$$5, ssrHydrationKey()), "RadioButtonChecked");

const _tmpl$$4 = ["<path", " d=\"M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8z\"></path>"];
/**
 * @ignore - internal component.
 */
var RadioButtonUncheckedIcon = createSvgIcon(() => ssr(_tmpl$$4, ssrHydrationKey()), "RadioButtonUnchecked");
...
function RadioButtonIcon(props) {
  ...
  return createComponent(RadioButtonIconRoot, {
    ...
    ownerState: ownerState,
    get children() {
      return [createComponent(RadioButtonIconBackground, {
        get fontSize() {
...
const $$h = createComponentFactory()({
  ...
  selfPropNames: ["checkedIcon", "classes", "color", "disabled", "icon", "size"],
...
const defaultCheckedIcon = () => createComponent(RadioButtonIcon, {
  checked: true
});
const defaultIcon = () => createComponent(RadioButtonIcon, {});
$$h.component(function Radio({
  classes,
  allProps,
  otherProps,
  props
  ...
  const checked = () => otherProps.checked ?? formControlLabelContext?.checked ?? areEqualValues$1(radioGroup?.value, value());
  ...
  return createComponent(SvgIconContext.Provider, {
    ...
    get children() {
      return createComponent(RadioRoot, mergeProps$1({
        type: "radio"
      }, otherProps, {
        ownerState: allProps,
        classes: classes,
...
$$g.component(function RadioGroup({
  props,
  otherProps
}) {
  ...
  return createComponent(RadioGroupContext.Provider, {
    ...
    get children() {
      return createComponent(FormGroup, mergeProps$1({
        role: "radiogroup"
      }, otherProps, {
        get children() {
          return otherProps.children;
...
const _tmpl$$3 = ["<span", " class=\"notranslate\">&#8203;</span>"];
const $$f = createComponentFactory()({
  name: "MuiSelectInput",
  ...
  slotClasses: ownerState => ({
    select: ["select", ownerState.variant, ownerState.disabled && "disabled", ownerState.multiple && "multiple"],
    icon: ["icon", `icon${capitalize(ownerState.variant)}`, ownerState.open && "iconOpen", ownerState.disabled && "disabled"],
...
const SelectInput = $$f.defineComponent(function SelectInput(props) {
  ...
  const action = {
    ...
    addEventListener(eventName, callback) {
      if (eventName !== "input") throw new Error(`Invalid event name: ${eventName}`);
  ...
  createEffect(on(() => [displayNode(), props.autoWidth], () => {
    if (props.defaultOpen && openState() && displayNode() && !isOpenControlled) {
  ...
  const items = mapArray(childrenArray, child => {
    if (isComponentObject(child, MenuItem)) {
      ...
      return createComponent$1(child.Component, childProps);
    } else {
      return isComponentObject(child) ? createComponent$1(child.Component, child.props) : child;
    }
  });
    ...
    if (isFilled({
      ...
      if (props.renderValue) {
        ...
        for (const itemProps of menuItemProps) {
          ...
          if (v.some(v => areEqualValues(v, itemProps.value))) result.push(itemProps.children);
        }
  ...
  const PaperProps = mergeProps(() => baseProps.MenuProps.PaperProps, {
    ...
    }, () => baseProps.MenuProps.PaperProps != null ? baseProps.MenuProps.PaperProps.style : null)
  });
  ...
  const buttonId = () => baseProps.SelectDisplayProps.id || (props.name ? `mui-component-select-${props.name}` : undefined);
  const ownerState = mergeProps(props, {
    get variant() {
      return baseProps.variant;
    },
  ...
  const displayValue = () => {
    ...
    ssr(_tmpl$$3, ssrHydrationKey()) : v;
  };
  return [createComponent(SelectSelect, mergeProps$1({
    get tabIndex() {
      return tabIndex();
    },
    role: "button",
    ...
    get ["aria-labelledby"]() {
      return [props.labelId, buttonId()].filter(Boolean).join(" ") || undefined;
    },
    get ["aria-describedby"]() {
      return props["aria-describedby"];
    },
    ...
    get ["class"]() {
      return clsx(classes.select, props.class, baseProps.SelectDisplayProps.class);
    },
    get id() {
      return buttonId();
    },
  ...
  })), createComponent(SelectNativeInput, mergeProps$1({
    get value() {
    ...
    ownerState: ownerState
  }, other)), createComponent(SelectIcon, {
    get as() {
      return props.IconComponent;
    },
    get ["class"]() {
    ...
    ownerState: ownerState
  }), createComponent(Menu, mergeProps$1({
    get id() {
...
const $$e = createComponentFactory()({
  name: "MuiSelect",
  selfPropNames: ["autoWidth", "children", "classes", "defaultOpen", "defaultValue", "displayEmpty", "IconComponent", "id", "input", "inputProps", "label", "labelId", "MenuProps", "multiple", "native", "onChange", "onClose", "onOpen", "open", "renderValue", "SelectDisplayProps", "value", "variant"]
});
 ...
 */
$$e.defineComponent(function Select(inProps) {
  const props = $$e.useThemeProps({
    props: inProps
  });
  const [, other] = splitProps(props, ["autoWidth", "children", "classes", "class", "defaultOpen", "displayEmpty", "IconComponent", "id", "input", "inputProps", "label", "labelId", "MenuProps", "multiple", "native", "onClose", "onOpen", "open", "renderValue", "SelectDisplayProps", "variant"]);
  const baseProps = mergeProps({
    autoWidth: false,
    classes: {},
    defaultOpen: false,
  ...
  const inputProps = mergeProps({
    ...
    get classes() {
      return props.inputProps ? deepmerge(classes, props.inputProps.classes) : classes;
    }
  ...
  return createComponent(Dynamic, mergeProps$1({
    ...
    inputProps: inputProps,
    get notched() {
      return props.multiple && props.native && variant() === "outlined" ? true : undefined;
    },
    get label() {
      return !inputElement() && variant() === "outlined" ? props.label : undefined;
    },
    get ["class"]() {
      return props.class;
    },
...
generateUtilityClasses("suidSkeleton", ["root", "text", "rectangular", "circular", "pulse", "wave", "withChildren", "fitContent", "heightAuto"]);

const $$d = createComponentFactory()({
  name: "MuiSkeleton",
  selfPropNames: ["animation", "children", "classes", "height", "variant", "width"],
  propDefaults: ({
    set
  }) => set({
    animation: "pulse",
  ...
  slotClasses: ownerState => ({
    root: ["root", ownerState.variant, ownerState.animation, ownerState.hasChildren && "withChildren", ownerState.hasChildren && !ownerState.width && "fitContent", ownerState.hasChildren && !ownerState.height && "heightAuto"]
  })
});
 ...
 */
$$d.component(function Skeleton({
  allProps,
  classes,
  otherProps,
  props
}) {
  const hasChildren = () => typeof props.children === "undefined" ? false : Array.isArray(props.children) ? !!props.children.length : true;
  const ownerState = mergeProps(allProps, {
    get hasChildren() {
      return hasChildren();
    }
  });
  return createComponent(SkeletonRoot, mergeProps$1(otherProps, {
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    ownerState: ownerState,
 ...
 */
$$c.component(function Stack({
  allProps,
  otherProps,
  props
}) {
  otherProps = extendSxProp(otherProps);
  return createComponent(StackRoot, mergeProps$1({
    get as() {
      return otherProps.component;
    },
    ownerState: allProps
...
const $$b = createComponentFactory()({
  name: "MuiSwitch",
  selfPropNames: ["checkedIcon", "classes", "color", "disabled", "icon", "size", "value"],
  propDefaults: ({
    set
  }) => set({
    color: "primary",
  ...
  slotClasses: ownerState => ({
    root: ["root", !!ownerState.edge && `edge${capitalize(ownerState.edge)}`, `size${capitalize(ownerState.size)}`],
    switchBase: ["switchBase", `color${capitalize(ownerState.color)}`, !!ownerState.checked && "checked", !!ownerState.disabled && "disabled"],
    thumb: ["thumb"],
    track: ["track"],
    input: ["input"]
  })
 ...
 */
$$b.component(function Switch({
  allProps,
  props,
  classes,
  otherProps
}) {
  const icon = () => createComponent(SwitchThumb, {
    get ["class"]() {
  ...
  return createComponent(SwitchRoot, {
    ...
    ownerState: allProps,
    get children() {
      return [createComponent(SwitchSwitchBase, mergeProps$1({
        type: "checkbox",
        ownerState: allProps
      }, baseProps, {
        classes: allClasses
...
$$a.component(function Table({
  ...
  return createComponent(TableContext.Provider, {
    ...
    get children() {
      return createComponent(TableRoot, mergeProps$1({
        get role() {
 ...
 */
$$9.component(function TableBody({
  allProps,
  classes,
  props,
  otherProps
  ...
  return createComponent(Tablelvl2Context.Provider, {
    ...
    get children() {
      return createComponent(TableBodyRoot, mergeProps$1({
        get role() {
          return otherProps.component === defaultComponent$3 ? null : "rowgroup";
        }
      }, otherProps, {
        get ["class"]() {
          return clsx(classes.root, otherProps.class);
...
const $$8 = createComponentFactory()({
  ...
  selfPropNames: ["align", "children", "classes", "component", "padding", "scope", "size", "sortDirection", "variant"],
  autoCallUseClasses: false,
  utilityClass: getTableCellUtilityClass,
  slotClasses: ownerState => ({
    root: ["root", ownerState.variant, ownerState.stickyHeader && "stickyHeader", ownerState.align !== "inherit" && `align${capitalize(ownerState.align)}`, ownerState.padding !== "normal" && `padding${capitalize(ownerState.padding)}`, `size${capitalize(ownerState.size)}`]
  })
});
const TableCellRoot = styled$1("td", {
  ...
  slot: "Root",
  overridesResolver: (props, styles) => {
    return [styles.root, styles[props.ownerState.variant], styles[`size${capitalize(props.ownerState.size)}`], props.ownerState.padding !== "normal" && styles[`padding${capitalize(props.ownerState.padding)}`], props.ownerState.align !== "inherit" && styles[`align${capitalize(props.ownerState.align)}`], props.ownerState.stickyHeader && styles.stickyHeader];
  }
})(({
  theme,
  ownerState
 ...
 */
$$8.component(function TableCell({
  otherProps,
  allProps,
  props
}) {
  ...
  const ariaSort = () => props.sortDirection ? props.sortDirection === "asc" ? "ascending" : "descending" : undefined;
  const ownerState = mergeProps(allProps, {
    get component() {
      return component();
    },
  ...
  return createComponent($TableCellRoot, mergeProps$1({
    get as() {
      return props.component;
    },
    get scope() {
 ...
 */
$$7.component(function TableContainer({
  classes,
  allProps,
  props,
  otherProps
}) {
  return createComponent(TableContainerRoot, mergeProps$1(otherProps, {
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    ownerState: allProps,
 ...
 */
$$6.defineComponent(function TableFooter(inProps) {
  ...
  return createComponent(Tablelvl2Context.Provider, {
    value: tablelvl2$1,
    get children() {
      return createComponent(TableFooterRoot, mergeProps$1({
        get as() {
          return baseProps.component;
        },
        get ["class"]() {
        ...
        get role() {
          return baseProps.component === defaultComponent$2 ? null : "rowgroup";
        },
        ownerState: ownerState
      }, other));
    }
 ...
 */
$$5.component(function TableHead({
  classes,
  allProps,
  otherProps,
  props
  ...
  return createComponent(Tablelvl2Context.Provider, {
    value: tablelvl2,
    get children() {
      return createComponent(TableHeadRoot, mergeProps$1({
        get role() {
          return otherProps.component === defaultComponent$1 ? null : "rowgroup";
        }
      }, otherProps, {
        get ["class"]() {
          return clsx(classes.root, otherProps.class);
...
const $$4 = createComponentFactory()({
  ...
  slotClasses: ownerState => ({
    root: ["root", ownerState.selected && "selected", ownerState.hover && "hover", ownerState.head && "head", ownerState.footer && "footer"]
  })
});
const TableRowRoot = styled$1("tr", {
  ...
  slot: "Root",
  overridesResolver: (props, styles) => {
    return [styles.root, props.ownerState.head && styles.head, props.ownerState.footer && styles.footer];
  }
})(({
  theme
}) => ({
...
$$4.component(function TableRow({
  ...
  return createComponent(TableRowRoot, mergeProps$1({
    get role() {
...
const $$3 = createComponentFactory()({
  ...
  selfPropNames: ["FormHelperTextProps", "InputLabelProps", "InputProps", "autoComplete", "autoFocus", "children", "classes", "color", "defaultValue", "disabled", "error", "fullWidth", "helperText", "id", "inputProps", "inputRef", "label", "maxRows", "minRows", "multiline", "name", "onBlur", "onChange", "onFocus", "placeholder", "required", "rows", "select", "size", "type", "value", "variant"],
 ...
 */
$$3.component(function TextField({
  allProps,
  classes,
  otherProps,
  props
  ...
  const InputElement = () => createComponent(Dynamic, mergeProps$1({
    get $component() {
      return InputComponent();
    },
    get ["aria-describedby"]() {
    ...
    get notched() {
      return props.variant === "outlined" && props.InputLabelProps && typeof props.InputLabelProps.shrink !== "undefined" ? props.InputLabelProps.shrink : undefined;
    },
    get label() {
  ...
  const label = () => {
    const label = props.label;
    return label != null && label !== "" && createComponent(InputLabel, mergeProps$1({
      get ["for"]() {
        return id();
      },
      get id() {
  ...
  const helperText = () => {
    ...
    if (helperText) return createComponent(FormHelperText, mergeProps$1({
      get id() {
        return helperTextId();
      }
  ...
  return createComponent(TextFieldRoot, mergeProps$1({
    get ["class"]() {
      return clsx(classes.root, otherProps.class);
    },
    get disabled() {
    ...
    get children() {
      return [label(), InputElement(), helperText()];
    }
  }));
});
...
const $$2 = createComponentFactory()({
  ...
  selfPropNames: ["children", "classes", "color", "disableFocusRipple", "disabled", "fullWidth", "onChange", "onClick", "selected", "size", "value"],
  propDefaults: ({
    set
  }) => {
  ...
  slotClasses: ownerState => ({
    root: ["root", !!ownerState.selected && "selected", ownerState.disabled && "disabled", ownerState.fullWidth && "fullWidth", `size${capitalize(ownerState.size)}`, ownerState.color]
  })
});
 ...
 * - inherits [ButtonBase API](https://mui.com/api/button-base/)
 */
$$2.component(function ToggleButton({
  allProps: _allProps,
  otherProps,
  props
}) {
  ...
  return createComponent(ToggleButtonRoot, mergeProps$1({
    get ["class"]() {
...
const $$1 = createComponentFactory()({
  ...
  selfPropNames: ["children", "classes", "color", "disabled", "exclusive", "fullWidth", "onChange", "orientation", "size", "value"],
  propDefaults: ({
    set
  }) => set({
    color: "standard",
  ...
  slotClasses: ownerState => ({
    root: ["root", ownerState.orientation === "vertical" && "vertical", ownerState.fullWidth && "fullWidth"],
    grouped: ["grouped", `grouped${capitalize(ownerState.orientation)}`, ownerState.disabled && "disabled"]
  })
});
 ...
 */
$$1.component(function ToggleButtonGroup({
  allProps,
  classes,
  otherProps,
  props
  ...
  return createComponent(ToggleButtonGroupContext.Provider, {
    ...
    get children() {
      return createComponent(ToggleButtonGroupRoot, mergeProps$1({
        role: "group",
        get ["class"]() {
          return clsx(classes.root, otherProps.class);
        },
...
const Toolbar = $.component(function Toolbar({
  ...
  return createComponent(ToolbarRoot, mergeProps$1(otherProps, {
    get as() {
      return otherProps.component;
    },
    get ["class"]() {
...
const Navbar = () => {
  ...
  const [appBarPositionStyle, setApBarPositionStyle] = createSignal('relative');
  let appBarRef;
  onMount(() => {
  ...
  return [createComponent(Box, {
    ...
    get children() {
      return [createComponent(Typography, {
        ...
        children: "Curtis Rochelle"
      }), createComponent(Typography, {
        variant: "h5",
        align: "center",
        color: "textSecondary",
        style: {
...
const ImageComponent = ({
  ...
  return createComponent(Box, {
    ...
    get children() {
      return [ssr(_tmpl$$2, ssrHydrationKey() + ssrAttribute("src", escape(src, true), false) + ssrAttribute("alt", escape(caption, true), false)), (caption || credit) && createComponent(Box, {
...
const _tmpl$$1 = ["<iframe", " width=\"100%\" height=\"auto\"", " allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share\" allowfullscreen style=\"", "\"></iframe>"];
...
const VideoComponent = ({
  ...
  return createComponent(RatioBox, {
    get sx() {
      return {
        ...sx
      };
    ...
    get children() {
      return ssr(_tmpl$$1, ssrHydrationKey(), ssrAttribute("src", escape(ytEmbedUrl, true), false) + ssrAttribute("title", escape(title, true), false), ssrStyle(`
          border: 0;
          position: absolute;
          top: 0;
          left: 0;
  ...
  return createComponent(Card, {
    ...
    get children() {
      return createComponent(Link, {
        ...
        get children() {
          return [createComponent(CardMedia, {
            component: "img",
            sx: {
              width: 175
            },
...
const GallerySlideComponent = props => {
  return createComponent(ImageComponent, props);
};

const GalleryComponent = ({
  slides
}) => {
  return slides.map(slide => createComponent(GallerySlideComponent, slide));
};

const _tmpl$ = ["<div", ">Not Found</div>"];
const PageComponent = ({
  component,
  width,
  sx,
  ...
  return createComponent(Grid, mergeProps$1({
    ...
    get children() {
      return createComponent(Switch, {
        ...
        get children() {
          return [createComponent(Match, {
            when: component === 'markdown',
            get children() {
              return createComponent(TextComponent, props);
            }
          }), createComponent(Match, {
            when: component === 'image',
            get children() {
              return createComponent(ImageComponent, props);
            }
          }), createComponent(Match, {
            ...
            get children() {
              return createComponent(VideoComponent, props);
            }
          }), createComponent(Match, {
            when: component === 'projectCard',
            get children() {
              return createComponent(ProjectCardComponent, props);
            }
          }), createComponent(Match, {
            when: component === 'gallery',
            get children() {
              return createComponent(GalleryComponent, props);
            }
          })];
        }
      });
...
const HeroComponent = props => {
  return createComponent(Grid, {
    ...
    get children() {
      return createComponent(PageComponent, mergeProps$1(props, {
        sx: {
          maxWidth: '768px'
        },
        width: "half"
  ...
  return createComponent(Container, {
    get children() {
      return createComponent(Box, {
        ...
        get children() {
          return ["\xA9 Copyright Curtis Rochelle ", copyrightYear];
        }
      });
    }
  });
...
const App = ({
  ...
  return [createComponent(Header, {}), createComponent(PageContainer, {
    get children() {
      return [!!pageData.hero && createComponent(HeroComponent, mergeProps$1(() => pageData.hero)), pageData.components.map(componentProps => createComponent(PageComponent, componentProps))];
    }
...
const render = (html, css, pageData) => {
  return `
    <!DOCTYPE html>
    <html>
      <head>
...
const renderPage = async (pageData, theme) => {
  const cache = createCache({
    key: 'css'
  });
  // const { extractCriticalToChunks, constructStyleTagsFromChunks } = createEmotionServer(cache);
  const fullTheme = createTheme$1(theme);

  // Render the components to a string.
  const html = await renderToStringAsync(() => createComponent(CacheProvider, {
    value: cache,
    get children() {
      return createComponent(ThemeProvider, {
        theme: fullTheme,
        get children() {
          return [createComponent(CssBaseline, {}), createComponent(App, {
            pageData: pageData
          })];
        }
      });
...
const req = async props => {
  ...
  const pageData = await getJson(`${PAGES_PATH}/${pageName}.json`);
  const theme = await getJson(`${DATA_PATH}/theme.json`);
